...
const logo = require('../../images/Cg2.png');
  ...
  return (
    ...
    <div className={`flex flex-col fixed -left-72 top-0 h-full w-72 overflow-y-scroll scrollbar-hide bg-blue-50 z-50 transform transition-transform ${isClickedOnHamberger ? 'translate-x-full' : 'translate-x-0'}`}>
      ...
      <div className="flex flex-col m-5 mt-3">
        <div className="account-section text-base font-medium flex flex-col my-3">
          ...
          {user && user.role !== 'admin' && <Link to={"/user/history"} className="my-3 text-xl font-normal text-[#010C80] hover:text-blue-800"><div>My Orders</div> </Link>}
    ...
    <div className={`${isClickedOnSearch ? 'hidden' : 'flex'} items-center shadow-md sticky top-0 bg-white z-40 md:py-2 md:justify-around`}>
      ...
      <div className="flex justify-center cursor-pointer items-center mx-5 absolute right-0 md:relative">
        ...
        <div onClick={() => setIsMouseOverOnAccount(!isMouseOverOnAccount)} className={`${isLoggedIn ? 'hidden' : 'flex'} md:flex flex-col justify-center items-center mx-2 text-xs font-semibold`}>
        ...
        {user && user.role !== 'admin' && <div className={`${isLoggedIn ? 'flex' : 'hidden'} flex-col justify-center items-center mx-2 text-xs font-semibold`}>
        ...
        {user && user.role !== 'admin' && <div className="flex flex-col justify-center items-center mx-2 text-xs font-semibold">
